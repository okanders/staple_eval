...
# Path to the JIT-compiled AIMNet2 model
AIMNet2_model_file = "AIMNet2/models/aimnet2_wb97m-d3_ens.jpt"

# Load the compiled AIMNet2 model with the appropriate device setting
if torch.cuda.is_available():
    # Load the model and use CUDA
    AIMNet2_model = torch.jit.load(AIMNet2_model_file).to(device)
    print('Using GPU')
else:
    # Load the model and use CPU
    AIMNet2_model = torch.jit.load(AIMNet2_model_file, map_location=device)
    print('Using CPU')
...
#NEW AIMNET
# Create an instance of the AIMNet2 calculator with the loaded model
aimnet_calculator = AIMNet2Calculator(AIMNet2_model)
...
# Move up one directory from the current script's location and then into 'b-hairpins-runs/02-06-2024'
#current_script_dir = os.path.dirname(os.path.abspath(__file__))  # Get the directory where the script is located

#parent_directory = os.path.join(current_script_dir, "..", "..", "..", "scratch", "okanders", "b-hairpin-runs", date_for_runs)  # Append the specific date
...
for name, smiles in name_smiles:
    ...
    #reminder to clear calculator cache
    aimnet_calculator.do_reset()
